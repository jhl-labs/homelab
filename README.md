...
## Homelab의 목적
- 집에 개발 클러스터를 구성하여 자유로운 개발 및 실험, 연구를 진행
...
## 스토리지 구성
- Proxmox 를 이용한 Ceph 클러스터
  - 3MON, 3MGR, 11 OSD 를 통한 클러스터 스토리지 제공 (nvme / hdd 기반 RWO, RWX 지원), s3 지원
...
## 주요 개발 방식
  ...
  - claude code를 통하여 다중 세션 (10~15개)를 윈도우 원격 환경에서 띄운 후 개발 진행  <-  세션이 안끊어짐
...
## 주요 개발 프로젝트
  ...
  - (비공개) Pics (인스타그램 컨셉 자체 사진 공유 플랫폼,  개발 목적은 Google Photos의 ai 분류 기능이 너무 보급형이라 가족 사진들(특히 어린이집 사진)을 제대로 인물 분류 / 장소 분류 시키게 하려고
  - (비공개) jhl-seach (내부 개발 리소스[github, confluence] 및 외부 개발 정보(hackersnews, google, meta 등)에 대한 크롤링 정보에 대한 opensearch 기반의 시맨틱 웹 검색 환경 제공.  자체적인 google 검색엔진 생태계를 만들고자 개발함 (RAG 연동 및 MCP 활용 목적)

## 이렇게 왜 하는가?
- 회사 내 커리어 변천사의 따른 기술 숙련 향상
  - 네트워크(WiFi 장비, 스위치 장비) 엔지니어 -> IoT 엔지니어 -> SE 품질 엔지니어 -> SE 인프라 엔지니어 -> DevOps 엔지니어 -> DevOps 에반젤리즘 -> 플랫폼 엔지니어 -> DevRel + 플랫폼 엔지니어
- 회사 내 부족한 환경에 대해 선진 문물(?) 배포에 대한 사명감 & 개인 능력 향상
- 언젠가 꿈꾸는 스타트업 창업을 위한 풀스택 기술 능력 확보 
